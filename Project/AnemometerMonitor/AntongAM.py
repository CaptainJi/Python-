...
now = time.strftime('%Y{y}%m{m}%d{d}%H{h}%M{M}').format(y='年', m='月', d='日', h='时', M='分')
...
def login():
    ...
    try:
        ...
        res_html = data_path + '登录异常' + now + '.html'
        ...
        driver.get_screenshot_as_file(screenshots_path + '状态截图' + now + '.png')

        page_source = driver.page_source
        res_html = data_path + '风表状态监测' + now + '.html'
        with open(res_html, mode='w', encoding='utf-8') as file:
            file.write(page_source)
        return res_html
...
def parss_data(res_html):
    ...
    try:
        ...
        result = []  # 构造列表
        # 循环取得每行中指定列的内容；.get_text(strip=True)为只取文字内容
        for tr in tr_arr[2:]:
            ...
            if online == '离线':
                ...
                warning = warning_path + '%s离线报警' % name + now + '.html'
...
def save_to_excel(data):
    ...
    # 输出保存
    book.save(reports_path + '风表监控' + now + '.xlsx')
    logging.info('数据已保存到/reports/风表状态监测' + now + '.xlsx')


# 获取最新警报文件
def latestReport():
    reportDir = warning_path
    # 获取文件列表
...
def latestScreenshot():
    screenshotDir = screenshots_path
    # 获取文件列表
...
if __name__ == '__main__':
    ...
    screenshots_path = path.join(path.dirname(path.abspath(__file__)), './screenshots/')
    warning_path = path.join(path.dirname(path.abspath(__file__)), './warning/')
    reports_path = path.join(path.dirname(path.abspath(__file__)), './reports/')
